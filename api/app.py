  ...
  def get(self,symbol,expiry):
    if(symbolexists(symbol)):
        if((len(expiry)==7) and (contains_only_numerical(expiry[0:2])) and (contains_only_letters(expiry[2:5])) and (contains_only_numerical(expiry[5:]))):
            all_expiry_for_symbol=All_expiry_symbols_meta.query.with_entities(All_expiry_symbols_meta.expiry_date).filter(All_expiry_symbols_meta.symbol==symbol,(All_expiry_symbols_meta.put==1 or All_expiry_symbols_meta.call==1)).order_by(All_expiry_symbols_meta.expiry_date).distinct()
...
# class Symbol_future_API(Resource):
#     def get(self,symbol):
#         if(symbolexists(symbol)):
#             symbol_futures_available=All_expiry_symbols_meta.query.filter(All_expiry_symbols_meta.symbol==symbol,All_expiry_symbols_meta.expiry_date,All_expiry_symbols_meta.future==1).first()
#             final=[]
#             if(symbol_futures_available):
#                 underlying_value=Indexes.query.with_entities(Indexes.LTP).filter(Indexes.symbol==symbol).first()
#                 if(underlying_value):
#                     symbol_futures=Futures.query.with_entities(Futures.symbol,Futures.expiry_date,Futures.LTP,Futures.prevClosePrice,Futures.LTQ,Futures.openInterest).filter(Futures.symbol==symbol).order_by(Futures.expiry_date).all()
#                     totalvolume=0
#                     totalvalue=0
#                     totaloi=0
#                     for i in symbol_futures:
#                         final.append({'instrument_type':'Index Futures','symbol':i.symbol,'expiry_date':date_converter_to_output_format(i.expiry_date.date()),'option_type':'-','strike':'-','ltp':i.LTP,'change':((i.LTP-i.prevClosePrice)/100),'percent_change':(((i.LTP-i.prevClosePrice)/(i.prevClosePrice))*100),'volume':i.LTQ,'value':(((i.LTP)/100)*i.LTQ),'oi':i.openInterest,'underlyingvalue':underlying_value.LTP})
#                         totalvolume=totalvolume+i.LTQ
#                         totalvalue=totalvalue+((i.LTP/100)*(i.LTQ))
#                         totaloi=totaloi+i.openInterest
#                     return({'data':final,'total_volume':totalvolume,'total_value':totalvalue,'total_oi':totaloi})
#                 else:
#                     return("E001 : True")
#             else:
#                 return("E001 : True")
...
# api.add_resource(Symbol_future_API,'/api/symbol_future/')
